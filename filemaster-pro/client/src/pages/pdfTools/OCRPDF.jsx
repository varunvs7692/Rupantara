...
export default function OCRPDF() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await apiFetch('/api/pdf-tools/ocr', {
        method: 'POST',
        body: formData
      });
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || 'Failed to OCR PDF');
      }
  ...
  return (
    <div className="max-w-lg mx-auto p-8">
      ...
      {downloadUrl && (
        <a href={downloadUrl} download className="block mt-4 text-green-600">Download OCR Result</a>
      )}
